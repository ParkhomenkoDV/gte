import os
from math import nan, inf, log, sqrt
...
from tools import isnum, rounding, eps, export2file, getProgressBar, run_thread_tasks_in_parallel
...
def input_GTE_scheme():
    ...
    for key in GTE_scheme:
        if key == 'Вл' and GTE_initial_data['m2'] == 0: GTE_scheme[key] = 0; continue
        if key == 'КС': GTE_scheme[key] = 1; continue
        if key == 'СII':
    ...
    for key in GTE_load:
        if key == 'ВД' and GTE_scheme['ТВД'] == 0: GTE_load[key] = 0; continue
        if key == 'СД' and GTE_scheme['ТСД'] == 0: GTE_load[key] = 0; continue
        if key == 'НД' and GTE_scheme['ТНД'] == 0: GTE_load[key] = 0; continue
        if key == 'СТ' and GTE_scheme['СТ'] == 0: GTE_load[key] = 0; continue
        while True:
            temp = input(key + ': ').strip()
            if temp == '':
                break
    ...
    for key in GTE_gear:
        if key == 'Вл' and GTE_scheme['Вл'] == 0: GTE_gear[key] = 0; continue

        if key == 'ВД' and GTE_scheme['ТВД'] == 0: GTE_gear[key] = 0; continue
        if key == 'СД' and GTE_scheme['ТСД'] == 0: GTE_gear[key] = 0; continue
        if key == 'НД' and GTE_scheme['ТНД'] == 0: GTE_gear[key] = 0; continue
...
def show():
    ...
    plt.plot((10 * w1, 11.5 * w1), (6 * w1, 6 * w1),
             (12.5 * w1, 14 * w1), (6 * w1, 6 * w1),
             (15 * w1, 16 * w1), (6 * w1, 6 * w1),
             (17 * w1, 18 * w1), (6 * w1, 6 * w1),
             ls='-', color=(1, 0.5, 0))
    plt.plot((8 * w1, 11.5 * w1), (4 * w1, 4 * w1),
             (12.5 * w1, 20 * w1), (4 * w1, 4 * w1),
             (21 * w1, 22 * w1), (4 * w1, 4 * w1),
             (23 * w1, 24 * w1), (4 * w1, 4 * w1),
             ls='-', color=(1, 1, 0))
    plt.plot((6 * w1, 11.5 * w1), (2 * w1, 2 * w1),
             (12.5 * w1, 26 * w1), (2 * w1, 2 * w1),
             (27 * w1, 28 * w1), (2 * w1, 2 * w1),
             (29 * w1, 30 * w1), (2 * w1, 2 * w1),
             ls='-', color=(0, 1, 0))
    plt.xlim(0, x)
    plt.ylim(-2 * h1, 2 * h1)
...
def input_degrees_of_increase_or_decrease_pressure():
    ...
    for key in GTE_ππ:
        if (key == 'ВлII' or key == 'ВлI') and key not in GTE_scheme: GTE_ππ[key] = 1; continue
        if key == 'СII' and key not in GTE_scheme: GTE_ππ[key] = 1; continue
        if key in GTE_scheme and GTE_scheme[key] == 0: GTE_ππ[key] = 1; continue
        while True:
            temp = input(key + ' = ').strip()
            if temp == '':
                break
...
def cycle_inlet_parameters(M=nan, T0=nan, P0=nan):
    ...
    return {'T*': TT_вх, 'P*': PP_вх,
            'Cp': Cp('AIR', TT_вх), 'k': k_вх}
...
def cycle_compressor(ηη=nan, m2=nan, ππI=nan, ππII=nan, PPI1=nan, PPII1=nan, TTI1=nan, TTII1=nan) -> dict:
    ...
    while True:
        ...
        if abs(Cp('AIR', T=TTI3) / (Cp('AIR', T=TTI3) - R_gas('AIR')) - kI3) / kI3 <= error: break
    ...
    while True:
        ...
        if abs(Cp('AIR', T=TTII3) / (Cp('AIR', T=TTII3) - R_gas('AIR')) - kII3) / kII3 <= error: break
    ...
    return {'iterationI': iterationI, 'iterationII': iterationII,
            'T*I1': TTI1, 'T*I3': TTI3, 'T*II1': TTII1, 'T*II3': TTII3,
            'P*I1': PPI1, 'P*I3': PPI3, 'P*II1': PPII1, 'P*II3': PPII3,
            'kI1': kI1, 'kI3': kI3, 'kII1': kII1, 'kII3': kII3,
            'π*I': ππI, 'π*II': ππII,
            'η*': ηη, 'η*n': ηηn, 'η*nI': ηηnI, 'η*nII': ηηnII,
            'L': L, 'LI': LI, 'LII': LII}
...
def cycle_combustion_chamber(H=nan, TT3=nan, TT1=nan, PP1=nan, fuel=''):
    ...
    g_fuel3 /= (Qa1(fuel) * GTE_ηη['гор'] -
                (1 + l_stoichiometry(fuel)) *
                (Cp('EXHAUST', fuel=fuel, a_ox=1, T=TT3) * TT3 - Cp('EXHAUST', fuel=fuel, a_ox=1, T=(T0 + 15)) * (
                        T0 + 15)) +
                l_stoichiometry(fuel) *
                (Cp('AIR', TT3)) * TT3 - Cp('AIR', (T0 + 15)) * (T0 + 15) +
                0)
    global g_fuel
    g_fuel = g_fuel3
...
def cycle_turbine(G=inf, N=0, ηη=nan, Lc=nan, PP1=nan, TT1=nan, g_cool=nan, TT_cool=nan, PP_cool=nan, fuel=''):
    ...
    while True:
        ...
        if abs(eps('rel',
                   Cp('EXHAUST', fuel=fuel, T=TT3, a_ox=α_ox) /
                   (Cp('EXHAUST', fuel=fuel, T=TT3, a_ox=α_ox) - R_gas('EXHAUST', fuel=fuel, a_ox=α_ox)),
                   k3)) <= error: break
...
def cycle_GTE(H=nan, M=nan, R=nan, TT_max=nan, m2=nan, ηη=nan, ππ=nan, σ=nan, fuel: str = '',
              trace=False) -> dict:
    """Расчет цикла ГТД"""

    GTE_initial_unperturbed_parameters = cycle_initial_unperturbed_parameters(H=H, M=M)
    if trace is True: print('Н', GTE_initial_unperturbed_parameters)

    GTE_inlet_parameters = cycle_inlet_parameters(M=M,
                                                  T0=GTE_initial_unperturbed_parameters['T'],
                                                  P0=GTE_initial_unperturbed_parameters['P'])
    if trace is True: print('Вх', GTE_inlet_parameters)

    GTE_КНД = cycle_compressor(ηη=ηη['КНД'], m2=m2,
                               ππI=ππ['КНДI'], ππII=ππ['КНДII'],
                               TTI1=GTE_inlet_parameters['T*'], TTII1=GTE_inlet_parameters['T*'],
                               PPI1=GTE_inlet_parameters['P*'] * σ['вх-КНД'],
                               PPII1=GTE_inlet_parameters['P*'] * σ['вх-КНД'])
    if trace is True: print('КНД', GTE_КНД)

    GTE_КСД = cycle_compressor(ηη=GTE_ηη['КСД'], m2=m2,
                               ππI=GTE_ππ['КСДI'], ππII=GTE_ππ['КСДII'],
                               TTI1=GTE_КНД['T*I3'], TTII1=GTE_КНД['T*II3'],
                               PPI1=GTE_КНД['P*I3'] * GTE_σ['КНД-КСД'], PPII1=GTE_КНД['P*II3'] * GTE_σ['КНД-КСД'])
    if trace is True: print('КСД', GTE_КСД)

    GTE_КВД = cycle_compressor(ηη=GTE_ηη['КВД'], m2=m2,
                               ππI=GTE_ππ['КВДI'], ππII=GTE_ππ['КВДII'],
                               TTI1=GTE_КСД['T*I3'], TTII1=GTE_КСД['T*II3'],
                               PPI1=GTE_КСД['P*I3'] * GTE_σ['КСД-КВД'], PPII1=GTE_КСД['P*II3'] * GTE_σ['КСД-КВД'])
    if trace is True: print('КВД', GTE_КВД)

    global g_fuel, α_ox, ππ_К
    g_fuel, α_ox, ππ_К = nan, nan, nan

    GTE_КС = cycle_combustion_chamber(H=H, TT1=GTE_КВД['T*I3'], TT3=TT_max, PP1=GTE_КВД['P*I3'] * GTE_σ['КС'],
                                      fuel=fuel)
    if trace is True: print('КС', GTE_КС)
    ...
    GTE_ТВД = cycle_turbine(ηη=GTE_ηη['ТВД'],
                            Lc=GTE_КВД['L'],
                            TT1=GTE_КС['T*I3'], PP1=GTE_КС['P*I3'] * GTE_σ['КС-ТВД'],
                            fuel=fuel)

    GTE_ТСД = cycle_turbine(ηη=GTE_ηη['ТСД'],
                            Lc=GTE_КСД['L'],
                            TT1=GTE_ТВД['T*3'], PP1=GTE_ТВД['P*3'] * GTE_σ['ТВД-ТСД'],
                            fuel=fuel)

    GTE_ТНД = cycle_turbine(ηη=GTE_ηη['ТНД'],
                            Lc=GTE_КНД['L'],
                            TT1=GTE_ТСД['T*3'], PP1=GTE_ТСД['P*3'] * GTE_σ['ТСД-ТНД'],
                            fuel=fuel)
    ...
    GTE_СТ = cycle_power_turbine(ηη=GTE_ηη['СТ'], ππ=ππ_СТ,
                                 TT1=GTE_ТНД['T*3'], PP1=GTE_ТНД['P*3'] * GTE_σ['ТНД-СТ'],
                                 fuel=fuel)
    ...
    GI /= (((GTE_ТВД['L'] + GTE_ТСД['L'] + GTE_ТНД['L'] + GTE_СТ['L']) * eff_мех) -
           (GTE_КНД['L'] + GTE_КСД['L'] + GTE_КВД['L']))
    if trace is True: print('GI (first iteration) (кг/с)', GI)

    while True:
        GTE_ТВД = cycle_turbine(G=GI, N=GTE_load['ВД'],
                                ηη=GTE_ηη['ТВД'],
                                Lc=GTE_КВД['L'],
                                TT1=GTE_КС['T*I3'], PP1=GTE_КС['P*I3'] * GTE_σ['КС-ТВД'],
                                fuel=fuel)
        if trace is True: print('ТВД', GTE_ТВД)

        GTE_ТСД = cycle_turbine(G=GI, N=GTE_load['СД'],
                                ηη=GTE_ηη['ТСД'],
                                Lc=GTE_КСД['L'],
                                TT1=GTE_ТВД['T*3'], PP1=GTE_ТВД['P*3'] * GTE_σ['ТВД-ТСД'],
                                fuel=fuel)
        if trace is True: print('ТСД', GTE_ТСД)

        GTE_ТНД = cycle_turbine(G=GI, N=GTE_load['НД'],
                                ηη=GTE_ηη['ТНД'],
                                Lc=GTE_КНД['L'],
                                TT1=GTE_ТСД['T*3'], PP1=GTE_ТСД['P*3'] * GTE_σ['ТСД-ТНД'],
                                fuel=fuel)
        if trace is True: print('ТНД', GTE_ТСД)
        ...
        GTE_СТ = cycle_power_turbine(ηη=GTE_ηη['СТ'], ππ=ππ_СТ,
                                     TT1=GTE_ТНД['T*3'], PP1=GTE_ТНД['P*3'] * GTE_σ['ТНД-СТ'],
                                     fuel=fuel)
        if trace is True: print('СТ', GTE_СТ)

        if trace is True: print('\n', 'g_fuel', g_fuel, '\n', 'α_ox', α_ox, '\n')

        GI_temp = (GTE_load['ВД'] / GTE_gear_eff['ВД'] +
                   GTE_load['СД'] / GTE_gear_eff['СД'] +
                   GTE_load['НД'] / GTE_gear_eff['НД'] +
                   GTE_load['СТ'] / GTE_gear_eff['СТ'])
        GI_temp /= (((GTE_ТВД['L'] + GTE_ТСД['L'] + GTE_ТНД['L'] + GTE_СТ['L']) * eff_мех) -
                    (GTE_КНД['L'] + GTE_КСД['L'] + GTE_КВД['L']))

        if abs(eps('rel', GI, GI_temp)) <= error: break
        GI = GI_temp
        if trace is True: print('GI (кг/с)', GI)

    GII = m2 * GI
    if trace is True: print('GII (кг/с)', GII)
    GΣ = GI + GII
    if trace is True: print('GΣ (кг/с)', GΣ)
    G_гор = GI * g_fuel
    if trace is True: print('G_гор (кг/с)', G_гор)
    Ne = 1

    Ce = G_гор / Ne
    if trace is True: print('Ce (кг/Вт)', Ce)
    ...
    return {'H_v': H, 'M_v': M, 'R_v': R, 'T*г': TT_max, 'm2': m2,
            'Ce': Ce, 'g_fuel': g_fuel, 'α_ox': α_ox,
            'GI': GI, 'GII': GII, 'GΣ': GΣ, 'G_гор': G_гор,
            **GTE_ππ}
...
def output_initial_data():
    ...
    for key in GTE_initial_data:
        if len(GTE_initial_data[key]) == 1:
            ...
            if GTE_initial_data[key][0] == GTE_initial_data[key][1]:
                ...
                print(key, ' = ', GTE_initial_data[key][0], ',',
                      GTE_initial_data[key][0] + (GTE_initial_data[key][1] - GTE_initial_data[key][0]) / (Ndis - 1),
                      '..', GTE_initial_data[key][1], sep='')
        elif len(GTE_initial_data[key]) == 3:
            print(key, ' = ', GTE_initial_data[key][0], ',',
                  GTE_initial_data[key][0] + (GTE_initial_data[key][1] - GTE_initial_data[key][0]) /
                  (GTE_initial_data[key][2] - 1),
                  '..', GTE_initial_data[key][1], sep='')
        else:
            print('ERORR')
    ...
    print(Fore.YELLOW + '\nСХЕМА ГТД:', sep=' ', end=' ')
    for key in GTE_scheme:
        if GTE_scheme[key] != 0: print(key, '', sep=' ', end='')
    print('\n')

    print(Fore.YELLOW + '\nНАГРУЗКА (Вт)\n')
    for key in GTE_load:
        if GTE_load[key] != 0: print(key, '=', GTE_load[key])
    print()

    print(Fore.YELLOW + '\nСТЕПЕНИ ПОВЫШЕНИЯ/ПОНИЖЕНИЯ ПОЛНОГО ДАВЛЕНИЯ\n')
    for key in GTE_ππ:
        if GTE_ππ[key] != 1: print(key, '=', GTE_ππ[key])
    print()
...
def generate_combinations(lst):
    ...
    if not lst:
        ...
        for value in rng(lst[0]):
            for combo in generate_combinations(lst[1:]): yield [value] + combo


def get_combinations(dct):
    all_comb = []
    keys, values = list(dct.keys()), list(dct.values())
    for combo in generate_combinations(values): all_comb.append(dict(zip(keys, combo)))
    return all_comb
...
if __name__ == '__main__':
    ...
    GTE_scheme = {'КНД': 0, 'КСД': 0, 'КВД': 1,
                  'КС': 1,
                  'ТВД': 1, 'ТСД': 0, 'ТНД': 0, 'СТ': 1,
                  'СI': 0, 'СII': 0}
    ...
    GTE_ηη = {'КНД': 0.86, 'КСД': 0.84, 'КВД': 0.82,
              'гор': 0.99,
              'ТВД': 0.82, 'ТСД': 0.84, 'ТНД': 0.86, 'СТ': 0.88}

    GTE_gear_eff = {'Вл': 0.97, 'ВД': 0.97, 'СД': 0.97, 'НД': 0.97, 'СТ': 0.97}

    # eff_мех, GTE_effeff, GTE_gear_eff = input_eff()

    GTE_σ = {'вх': 0.94,  # 0.93 <= σ_вх<= 0.99
             'вх-КНД': 1, 'КНД-КСД': 1, 'КСД-КВД': 0.99, 'КВД-КС': 0.99,  # 0.96 <=σ <=0.97
             'КС': 0.99,
             'КС-ТВД': 0.99, 'ТВД-ТСД': 1, 'ТСД-ТНД': 1, 'ТНД-СТ': 0.99,
             'СТ-СI': 0.99, 'КНД-СII': 0.99,
             'СI': 1, 'СII': 1,
             'вых': 0.98}  # 0.98<=σ_вых<=0.99

    GTE_g_leak = {'КНДI': 0, 'КНДII': 0,
                  'КСДI': 0, 'КСДII': 0,
                  'КВДI': 0, 'КВДII': 0,
                  'КС': 0,
                  'ТВД': 0, 'ТСД': 0, 'ТНД': 0,
    ...
    for c in tqdm.tqdm(get_combinations(GTE_initial_data), desc='Loading', ncols=70):
        '''print(cycle_GTE(H=c['H_v'], M=c['M_v'],
                        ...
                        fuel=c['fuel']))'''
        df = pd.concat((df,
                        pd.DataFrame(cycle_GTE(H=c['H_v'], M=c['M_v'], R=c['R_v'],
                                               TT_max=c['T*г'], m2=c['m2'],
                                               ηη=GTE_ηη, ππ=GTE_ππ, σ=GTE_σ,
                                               fuel=c['fuel']),
                                     index=[0])),
