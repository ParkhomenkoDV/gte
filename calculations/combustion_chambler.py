...
from thermodynamics import T0, atmosphere_standard, Cp, R_gas, Qa1, l_stoichiometry, g_cool_BMSTU
...
class CombustionChamber:
    ...
    def set_combination(self, combination, combustionchamber_main) -> None:
        ...
        varible_params = [key for key, value in combustionchamber_main.__dict__.items()
                          if type(value) is list and len(value) and not key.startswith('_')]
        positions = [0] * len(varible_params)

        for i in range(combination):
            for j, param in enumerate(varible_params):
                if positions[j] == len(getattr(combustionchamber_main, varible_params[j])) - 1:
                    positions[j] = 0
                else:
                    positions[j] += 1
                    continue
        for j, param in enumerate(varible_params):
            setattr(self, varible_params[j], getattr(combustionchamber_main, varible_params[j])[positions[j]])
    ...
    def get_outlet_parameters(self, how='all') -> None:
        ...
        if hasattr(self, 'TT3'):  # для КС
            if self.TT1 > self.TT3: self.warnings[3].add('T*1_КС > T*3_КС!'); return
            assert hasattr(self, 'η_burn'), f'{type(self).__name__} object has no attribute η_burn!'
            assert hasattr(self, 'T_fuel'), f'{type(self).__name__} object has no attribute T_fuel!'
            g_fuel3 = Cp(self.substance, T=self.TT3, P=self.PP3,
                         a_ox=getattr(self, 'a_ox1', None), fuel=fuel) * self.TT3
            g_fuel3 -= self.Cp1 * self.TT1
            g_fuel3 /= (Qa1(fuel) * self.η_burn -
                        ...
                        (Cp(self.substance, self.TT3)) * self.TT3 - Cp(self.substance, (T0 + 15)) * (T0 + 15) +
                        (Cp(fuel, T=self.T_fuel) * self.T_fuel - Cp(fuel, T=T0 + 15) * (T0 + 15)))
            self.a_ox3 = 1 / g_fuel3 / l_stoichiometry(fuel)
            if self.a_ox3 < 1: self.warnings[1].add('a_ox < 1! Топливо не сгорает полностью!')
        ...
        if scheme:
            for node in range(n): g_leaks += scheme[c][node].g_leak
        assert hasattr(self, 'T_lim'), f'{type(self).__name__} object has no attribute T_lim!'
        g_cools = g_cool_BMSTU(self.TT3, T_lim=self.T_lim)  # суммарные массовый расход на охлаждение после КС
    ...
    def __calculate(self, how='all', **kwargs) -> None:
        ...
        assert fuel, f'{CombustionChamber.__name__}.{CombustionChamber.solve.__name__}() method has no arguments fuel!'
...
if __name__ == '__main__':
    ...
    for k, v in cc.__dict__.items(): print(k, '=', v)
