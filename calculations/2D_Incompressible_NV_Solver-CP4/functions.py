...
def creat_obj(const, obj):
    ...
    if obj.sort == 'hemicircle':
        ...
        obj.cgrid = (((const.X_ave - cx) ** 2 + (factorxy * const.Y_ave - factorxy * cy) ** 2 <= R ** 2) * (
                    -(const.Y_ave - (cy + translation_c)) * math.tan(theta) <= const.X_ave - (cx + translation_c))).T
...
def naca4(number, x, finite_TE=False):
    ...
    yt = [5 * t * (a0 * math.sqrt(xx) + a1 * xx + a2 * math.pow(xx, 2) + a3 * math.pow(xx, 3) + a4 * math.pow(xx, 4))
          for xx in x]
...
def check_energy(data):
    ...
    """
    return 1 / 2 * (np.sum(np.sum(data.U ** 2)) / (np.shape(data.U)[0] * np.shape(data.U)[1]) + np.sum(
        np.sum(data.V ** 2)) / (np.shape(data.V)[0] * np.shape(data.V)[1]))
