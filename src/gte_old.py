...
class GTE:
    ...
    def version(cls) -> str:
        ...
        next_version = (
            ...
            'type(node) is class -> isinstance(node, class)ТД параметры через PTM 1677 as {"O2": 98, "H2": 2}',
    ...
    def get_variability(self) -> int:
        ...
        return prod([len(value) for key, value in self.__dict__.items() if type(value) is list and len(value) and not key.startswith("_")])
    ...
    def __set_combination(self, combination, gte_main) -> None:
        ...
        varible_params = [key for key, value in gte_main.__dict__.items() if type(value) is list and len(value) and not key.startswith("_")]
                    ...
                    if node in ("CONTOUR", "КОНТУР"):
                        ...
                        if self.scheme[contour]:
                            while True:  # ввод положения в контуре
                                place = input("Input the place in scheme GTE from 1 to " + str(len(self.scheme[contour]) + 1) + ": ")
                                if isnum(place, type_num="int") and 1 <= int(place) <= len(self.scheme[contour]) + 1:
                                    place = int(place)
                                    break
                                else:
                                    print(Fore.RED + "place is int num in [1..{}]!".format(len(self.scheme[contour]) + 1))
                        ...
                        if self.loads:
                            while True:
                                place = input("Input the place in GTE loads from 1 to " + str(len(self.loads) + 1) + ": ")
    ...
    def input_node_parameters(self) -> None:
        ...
        for contour in self.scheme:
            print(
                "Parameters of nodes of " + Fore.MAGENTA + "contour " + to_roman(contour),
                ":",
                sep="",
            )
            for node in self.scheme[contour]:
    ...
    def __calculate(self, how="all", error: float = 0.01, Niter: int = 100, **kwargs):
        ...
        Cx_1 = 2.0005 * 10 ** (-2) * (m - 1) ** 0 - 0.1879 * 10 ** (-3) * (m - 1) ** 1 + 2.8571 * 10 ** (-5) * (m - 1) ** 2 - 3.4667 * 10 ** (-7) * (m - 1) ** 3
        # сопротивление обечайки ГТД
        Cx_ = 3.5606 * 10 ** (-2) * (m - 1) ** 0 - 1.2744 * 10 ** (-3) * (m - 1) ** 1 + 3.3429 * 10 ** (-5) * (m - 1) ** 2 - 3.6667 * 10 ** (-7) * (m - 1) ** 3
            ...
            data.append(
                {
                    ...
                    **dict(
                        zip(
                            list(f"contour{to_roman(contour)}" for contour in gte_var.scheme),
                            list("+".join(list(type(node).__name__ + f"_{node.name}" for node in v)) for v in gte_var.scheme.values()),
                        )
                    ),
                    # нагрузка ГТД
                    "loads": "+".join(list(type(node).__name__ + f"_{node.name}" for node in gte_var.loads)),
                    # степень контурности контуров
                    **dict(
                        zip(
                            list(f"m{to_roman(contour)} []" for contour in gte_var.scheme),
                    ...
                    **dict(
                        zip(
                            list(f"G{to_roman(contour)} [кг/с]" for contour in gte_var.scheme),
                    ...
                    **dict(
                        zip(
                            list(type(node).__name__ + f"_{node.name} " + k for contour in gte_var.scheme for node in gte_var.scheme[contour] for k, v in node.__dict__.items()),
                            list(v for contour in gte_var.scheme for node in gte_var.scheme[contour] for k, v in node.__dict__.items()),
                        )
                    ),
                    # нагрузка ГТД
                    **dict(
                        zip(
                            list(type(node).__name__ + f"_{node.name} " + k for node in gte_var.loads for k, v in node.__dict__.items()),
                            list(v for node in gte_var.loads for k, v in node.__dict__.items()),
                        )
                    ),
                }
            )
        ...
        data = {  # схема ГТД
            **dict(
                zip(
                    ...
                    list("+".join(list(type(node).__name__ + f"_{node.name}" for node in v)) for v in self.scheme.values()),
                )
            ),
            # нагрузка ГТД
            ...
            **dict(
                zip(
                    list(type(node).__name__ + f"_{node.name} " + k for contour in self.scheme for node in self.scheme[contour] for k, v in node.__dict__.items() if type(v) is list),
                    list(str(v) for contour in self.scheme for node in self.scheme[contour] for k, v in node.__dict__.items() if type(v) is list),
                )
            ),
            **dict(
                zip(
                    list(type(node).__name__ + f"_{node.name} " + k for node in self.loads for k, v in node.__dict__.items() if type(v) is list),
                    list(str(v) for node in self.loads for k, v in node.__dict__.items() if type(v) is list),
                )
            ),
        }
        pd.set_option("display.max_row", None)
